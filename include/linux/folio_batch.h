...
/**
 * folio_add_lru_cache() - Add a folio to lru_cache
 * @fbatch: The folio batch.
 * @folio: The folio to add.
 *
 * The folio is added to the end of the batch.
 ...
 */
static inline unsigned int folio_add_lru_cache(struct folio_batch *fbatch,
		struct folio *folio)
{
	unsigned long nr_pages = folio_nr_pages(folio) + fbatch->nr_pages;

	fbatch->folios[fbatch->nr++] = folio;
	fbatch->nr_pages = min_t(unsigned long, nr_pages, FOLIO_BATCH_SIZE + 1);
